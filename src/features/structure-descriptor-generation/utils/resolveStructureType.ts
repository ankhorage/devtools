...
function resolveNamedType(
  ...
  if (owner && owner !== context.packageName) {
    return { kind: 'ref', packageName: owner, id };
  }
...
/*** Resolve primitives, literals, arrays, unions, and anonymous/fixed object shapes. */
function resolveTypeBody(type: ts.Type, context: StructureCompilerContext): StructureDescriptor {
  ...
  if (arrayItem) return { kind: 'ordered-list', item: resolveStructureType(arrayItem, context) };
...
function resolveArrayItem(type: ts.Type, checker: ts.TypeChecker): ts.Type | null {
  ...
  const targetName = reference.target?.symbol?.getName() ?? type.symbol?.getName();
  if (targetName !== 'ReadonlyArray') return null;
  return checker.getTypeArguments(reference).at(0) ?? null;
}

/*** Resolve a fixed object shape and reject unmarked string-index maps and executable objects. */
...
function resolveObjectField(
  property: ts.Symbol,
  context: StructureCompilerContext,
): readonly [string, { readonly value: StructureDescriptor; readonly optional?: boolean }] {
...
function resolveUnionDiscriminator(
  ...
  const candidates = first.getProperties().map((property) => property.getName()).sort();

  for (const name of candidates) {
    const values = variants.map((variant) => resolveDiscriminatorValue(variant, name, checker));
    if (values.every((value) => value !== null) && new Set(values).size === values.length) return name;
  }
  return null;
}
...
function stableSymbolName(symbol: ts.Symbol): string {
  const name = symbol.getName();
  if (!name || name.startsWith('__')) throw new Error('Structural descriptor symbols need stable names.');
  return name;
}

/*** Sort finite scalar values deterministically without changing their scalar identity. */
...
/*** Create one actionable unsupported-type error without exposing absolute source paths. */
